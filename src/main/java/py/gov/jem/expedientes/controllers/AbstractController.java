...
import py.gov.jem.expedientes.facades.LazyEntityDataModel;
...
public abstract class AbstractController<T> implements Serializable {
    ...
    private boolean errorPersistencia;

    public boolean isErrorPersistencia() {
        return errorPersistencia;
    }

    public void setErrorPersistencia(boolean errorPersistencia) {
        this.errorPersistencia = errorPersistencia;
    }
    ...
    public void save(ActionEvent event) {
        errorPersistencia = false;
        //String msg = ResourceBundle.getBundle("/Mensajes").getString(itemClass.getSimpleName() + "Updated");
    ...
    public void saveNew(ActionEvent event) {
        errorPersistencia = false;
        //String msg = ResourceBundle.getBundle("/Mensajes").getString(itemClass.getSimpleName() + "Created");
        String msg = "Creado";
    ...
    public void saveNew2(ActionEvent event) {
        // String msg = ResourceBundle.getBundle("/Mensajes").getString(itemClass.getSimpleName() + "Created");
        String msg = "Creado";
        persist2(PersistAction.CREATE, msg);
        if (!isValidationFailed()) {
            items = null; // Invalidate list of items to trigger re-query.
            lazyItems = null; // Invalidate list of lazy items to trigger re-query.
        }
    ...
    public void delete(ActionEvent event) {
        // String msg = ResourceBundle.getBundle("/Mensajes").getString(itemClass.getSimpleName() + "Deleted");
        String msg = "Borrado";
                ...
                if (cause != null) {
                    if (cause instanceof ConstraintViolationException) {
                        if (persistAction == PersistAction.DELETE) {
                            JsfUtil.addErrorMessage("El registro no puede ser borrado pues existen registros asociados al mismo.");
                        } else {
