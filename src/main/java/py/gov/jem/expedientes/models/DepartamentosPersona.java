...
@XmlRootElement
@NamedQueries({
    @NamedQuery(name = "DepartamentosPersona.findAll", query = "SELECT p FROM DepartamentosPersona p")
    , @NamedQuery(name = "DepartamentosPersona.findById", query = "SELECT p FROM DepartamentosPersona p WHERE p.id = :id")
    , @NamedQuery(name = "DepartamentosPersona.findAllButNull", query = "SELECT p FROM DepartamentosPersona p WHERE p.codigoCorte is not null ORDER BY p.descripcion")
...
public class DepartamentosPersona implements Serializable {
    ...
    public String toString() {
        return "py.gov.jem.expedientes.models.DepartamentosPersona[ id=" + id + " ]";
    }
