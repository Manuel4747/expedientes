...
@NamedQueries({ // ((select count(a.id) from ExpPersonasFirmantesPorActuaciones a WHERE a.documentoJudicial.id = d.id and a.estado.codigo = 'AC' AND a.personaFirmante = :personaFirmante AND a.actuacion.estado.codigo in ('FM', 'RP', 'FP')) > :limite)
    ...
    , @NamedQuery(name = "ExpActuaciones.findParaLaFirmaFiltarPorFechaPresentacion", query = "SELECT t.actuacion FROM ExpPersonasFirmantesPorActuaciones t, ExpActuaciones a WHERE t.actuacion.id = a.id and t.estado = :estado AND t.actuacion.fechaPresentacion >= :fechaDesde AND t.actuacion.fechaPresentacion <= :fechaHasta  AND t.firmado = false and a.formato is null ORDER BY t.actuacion.fechaPresentacion DESC")
    , @NamedQuery(name = "ExpActuaciones.findParaLaFirmaFiltarPorFechaResolucion", query = "SELECT t.actuacion FROM ExpPersonasFirmantesPorActuaciones t join t.actuacion a left join a.resolucion r WHERE t.estado = :estado AND r.fecha >= :fechaDesde AND r.fecha <= :fechaHasta  AND t.firmado = false and a.formato is null ORDER BY r.fecha DESC")
    , @NamedQuery(name = "ExpActuaciones.findByHash", query = "SELECT t FROM ExpActuaciones t WHERE t.hash = :hash")
    ,@NamedQuery(name = "ExpActuaciones.findByBuscarPorFechaPresentacion", query = "SELECT t FROM ExpActuaciones t WHERE t.fechaPresentacion >= :fechaDesde AND t.fechaPresentacion <= :fechaHasta  ORDER BY t.fechaPresentacion ASC")
